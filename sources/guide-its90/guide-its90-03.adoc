...
In the null mode, temperature and line-pressure dependence, hysteresis, and stability of the zero are the limiting factors. High linearity is desirable for ease of calibration but is not essential. In this mode, the resolution may approach stem:[1 "unitsml(pp)""unitsml(m)"] of stem:[100 "unitsml(kPa)"] and the temperature coefficient of the zero about stem:[1 "unitsml(Pa/K)"]. Repeatability is improved by pre-stressing the diaphragm in a given direction at a pressure corresponding to full scale deflection and taking care that afterwards the pressure never exceeds this value nor changes sign.
