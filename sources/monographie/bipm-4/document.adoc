...
An instrument can be adjusted to show the activity reading stem:[ii(R)_"N" = ii(A)] for a defined radionuclide stem:["N"] directly in units of becquerel or its multiples (kBq, MBq, etc.). The adjustment condition is stem:[g_(cancel ("N")) ii(epsilon)_"N" = 1] and it follows that
...
It is important that calibrations and measurements use solutions of the same density. When carrier solutions are used for a dilution with concentrations below stem:[50" "rm(mu) rm(g)] salt per gram of solution and have low acid concentration, the density of the radioactive solution remains close to 1 g/ml. This corresponds to standard calibration conditions so additional correction factors for attenuation of the photon radiation are not required.
...
|===
^h| Radio-nuclide ^h| stem:[ii(T)_(1//2)] ^h| stem:[ii(E)] keV ^h| stem:[ii(N)] (10^3^ counts) ^h| stem:[ii(p)] ^h| stem:[ii(eta)] (stem:[*10^(-3)]) ^h| stem:[ii(A)] (kBq) ^h| stem:[ii(A)//ii(A)_0] (%)
...
|===
| Nuclide | stem:[ii(E)] (keV) | stem:[p] | stem:[ii(epsilon)_"N"] | stem:[ii(epsilon)] (GBq^-1^) | stem:[ii(epsilon)_"N"^("abs")] | stem:[ii(epsilon)^("abs")] | Comments
...
| ^22^Na | 511.0 | 1.798 | 72.7 | 19.0 | 72.7 | 19.0 | 1274.5 keV footnote:fb[One or more stem:[p ii(epsilon)] have been subtracted (see text);]
...
.2+| Laboratory .2+| Ampoule number .2+| Method of standardization .2+| Reference date .2+| Activity at ref. date (kBq) 2+| Rel. uncert. Category .2+| Date | Relative uncertainty | stem:[ii(A)_"e"] | Combined uncert. of stem:[ii(A)_"e"]
| A (stem:[r_1],%) | B (stem:[r_2], %) | (stem:[r_3],%) | (kBq) | stem:[ii(A)_"e"(r_1^2 + r_2^2 + r_3^2)^(1//2)] (kBg)
...
====
[[fig64a]]
.Photon efficiency stem:[ii(epsilon)_1] versus photon energy for the PTB ionization chamber, type IG 12 of Centronic (see <<fig14>>). The dashed line shows the chosen linear efficiency function stem:[ii(epsilon) = 0.45" "ii(E) + 0.1] for reduction (energy stem:[ii(E)] in MeV). After Weiss (<<wei1982>>).
image::img114.png[]
