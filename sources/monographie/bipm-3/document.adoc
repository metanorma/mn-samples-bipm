...
In the region above 10 keV, stem:[ii(B)] depends on shielding, construction materials used
...
this discussion, stem:[ii(eta)] values will be for stem:[ii(E)_"e" = 5" "rm(keV)] unless otherwise stated.
...
[%unnumbered]
|===
.2+| Scintillator 2+| Relative pulse height .2+| stem:[ii(lambda)_("max")" "rm(nm)]
| Al | TiO~2~
| TP | 0.91 | 0.76 | 342
| PPO | 1.02 | 0.97 | 375
| POPOP | 1.21 | 1.45 | 415
...
stem:[n_"D" = 1.50] and a kinematic viscosity of 1.15 cm^2^ s^-l^ <<dow79>>.
...
[[tableIII-1]]
[cols="^,^,^"]
.Typical figures of merit for commercial counting system footnote:[Similar systems have been reported using EMI phototubes, e.g. EMI QB9635 for a commercial system <<sta74-2>>, and EMI 95145/A for a high-efficiency LS counting system <<ste67-1>>.] and for high-efficiency lS counting system for stem:[ii(E)_"e" = 5" "rm(keV)]
|===
...
| stem:[g_"c"] | 0.80 | 0.80
a| stem:[ii(eta)]: 5 keV for each phototube footnote:[stem:[ii(eta)_0] values taken from <<figIII-2>> were corrected for ionization quenching using stem:[ii(Q)(ii(E)) = 0.63] (see <<figIV-6>>.] | 0.37 | 0.38
...
The figures of merit stem:[ii(eta)_(49) " "rm(keV)] of about 0.3 photoelectron/keV for each tube suggest,
...
so that an input of stem:[ii(E)" "rm(keV)] will produce, on average, a pulse of height stem:[bar(ii(V))] volts
...
where stem:[ii(eta)] is the figure of merit (photoelectrons/keV) and stem:[ii(E)] is the energy in stem:[rm(keV)].
...
where stem:["d"ii(E)//"d"x] is the specific energy loss (normally expressed in MeV cm^-2^ g^-1^),
...
the constant stem:[k ii(B)] is about stem:[9" "rm(mg)" "rm(cm)^(-2)" "rm(MeV)^(-l)] and thus for stem:[beta] particles and electrons
with energies greater than about stem:[1" "rm(keV)], where stem:["d"ii(E)//"d"x = 100" "rm(MeV)" "rm(cm)^(-2)" "rm(g)^(-l)],
...
stem:[k ii(B) = 9" "rm(mg)" "rm(cm)^(-2)" "rm(MeV)^(-l)] is given in <<figIV-6>>, which was calculated using a modified
form of the Bethe-Bloch theory for stem:["d" ii(E)//"d"x] <<nel56>>, with stem:["d" ii(E)//"d"x] proportional
to stem:[ii(E)^(-0.5)] below stem:[400" "rm(eV)] <<mar70-1>>.
...
The value of stem:[1 - ii(J)(ii(E))] for a stem:[15" "rm(cm)^3] volume with stem:[a = 10.5" "rm(mm)] and stem:[l = 21.75" "rm(mm)]
...
value of stem:[ii(Q)(ii(E))] for an energy of 5 keV. The quality parameter measured on their
...
The sharpness of the peaks in the afterpulse distribution below stem:[1" "rm(mu) rm(s)] has been
...
prompt afterpulse (at about stem:[0.3" "rm(mu) rm(s)] at typical voltages) which is observed in
...
no afterpulses. Shin <<shi68>> also observed afterpulses in the region below stem:[1" "rm(mu) rm(s)]
...
afterpulses at 0.8 and stem:[4" "rm(mu) rm(s)], following single photoelectron events, and suggested
...
up to stem:[15" "rm(mu) rm(s)] after the main pulse, with an intensity correlated to the main pulse,
...
a variable discriminator, it was found that if any region (above about stem:[1" "rm(mu) rm(s)]) of
...
excited by ^60^Co, and observed a slow component of order stem:[0.1" "rm(mu) rm(s)] for toluene
...
liquid scintillators, Kuchnir <<kuc68>> observed slow components of order stem:[0.5" "rm(mu) rm(s)],
...
stem:[gamma]-ray-induced phosphorescence (stem:[ii(tau) < 30 rm(s)]) at discrimination levels equivalent
to one photoelectron.

With electron excitation, Owen <<owe59>> observed a slow component in oxygen-free
liquid scintillators with a time constant of the order of stem:[1" "rm(mu) rm(s)], and Horrocks
<<hor70-1>> gives time constants of about stem:[0.25" "rm(mu) rm(s)]. The relative intensities of the
...
<<bir64-1>> (stem:[<= 0.1%] that of the main pulse after stem:[1" "rm(mu) rm(s)]) and they can be avoided
...
In the region below stem:[100" "rm(mu) rm(s)], the degree of afterpulsing can be readily observed
by variation of the dead time, and such experiments at NPL indicate that a dead
time of at least stem:[10" "rm(mu) rm(s)] is required to reduce these spurious pulses to well below
...
In the stem:[10" "rm(mu) rm(s)] to stem:[100" "rm(ms)] range a correlation technique <<gol55>> has been used
...
(one) | RCA 31000D +
(one)
| stem:[beta]-dead time (stem:[rm(mu) rm(s)beta]) | 5.0 | 1.2 | live timer used | 24.0
| stem:[gamma]-dead time (stem:[rm(mu) rm(s)gamma]) | 5.0 | 1.1 to 3.7 | 1.4 to 2.0 | 1.7 to 4.4
| Resolving time stem:[rm(mu) rm(s)] | 1.0 | 0.5 | 1.8 | 0.2
...
.Results
|===
.2+| .2+a| Coincident stem:[ii(E)_gamma] or stem:[Delta ii(E)_gamma] (MeV) .2+a| Range of stem:[ii(N)_"c"//ii(N)_gamma] .2+a| stem:[ii(N)_0] (stem:[rm(Bq)*rm(mg)^(-1)]) 2+a| Percent. uncertainty .2+a| Degree of fit
